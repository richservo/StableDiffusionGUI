...
from BSRGAN_main.utils import utils_logger
from BSRGAN_main.utils import utils_image as util
# from utils import utils_model
from BSRGAN_main.models.network_rrdbnet import RRDBNet as net
...
#    print(torch.backends.cudnn.version())  # cudnn version
    ...
    for model_name in model_names:
        if model_name in ['BSRGANx2']:
            sf = 2
        model_path = os.path.join('BSRGAN_main\\model_zoo', model_name+'.pth')          # set model path
...
#            model.load_state_dict(state_dict, strict=True)
        ...
        # --------------------------------
        # idx += 1
        # img = 'testsets\\RealSRSet\\00913-35_k_euler_846569542_a_junkie_by_Paul_Cadden_pencil_drawing_photoreal_sharp_focus_masterpiece_post_processing.png'
        # E_path = 'testsets\\RealSRSet\\'
        img_name, ext = os.path.splitext(os.path.basename(img))
        print('upscaling ' + img_name)
        ...
        if save_results:
            ...
            img = Image.open(os.path.join(E_path, img_name+'_'+model_name+'.png'))
            img.show()
